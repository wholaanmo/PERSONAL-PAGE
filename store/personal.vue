      ...
      <div v-if="!isAddingBudget && !isEditingBudget" class="budget-display">
        ...
        <div class="budget-form-buttons"> <!--NEWWWWWWWW-->
          ...
          <div class="form-group">
            ...
            <select v-model="selectedMonthYear" @change="handleMonthYearChange($event.target.value)"> <!--NEWWWW-->
            <option v-for="month in availableMonths" :key="month" :value="month">{{ formatMonthYear(month) }}</option>
            </select>
          </div>
          <div class="form-group">
  ...
  methods: {
    ...
    async submitBudget() {
      try {
        ...
        const action = this.isEditingBudget ? 'updateBudget' : 'addBudget';
        const { success, message } = await this[action](budgetData);
        
        if (success) {
          this.showBudgetSuccessMessage(message || 'Budget operation successful!');
          this.cancelBudgetForm();
        } else {
          throw new Error(message);
        }
      } catch (error) {
        this.showBudgetSuccessMessage(error.message || 'Budget operation failed');
      }
    },
    ...
    async handleSubmit() {
      ...
      const expenseData = {
        ...
        personal_budget_id: this.getCurrentBudget?.id
      };
    ...
    // Shared UI Methods
    handleTimedMessage(type, message) {
      const propPrefix = `${type}Success`;
      const timeoutProp = `${propPrefix}Timeout`;
      
      if (this[timeoutProp]) clearTimeout(this[timeoutProp]);
      
      this[`${propPrefix}Message`] = message;
      this[`${type}HideMessage`] = false;
      
      this[timeoutProp] = setTimeout(() => {
        this[`${type}HideMessage`] = true;
        setTimeout(() => this[`${propPrefix}Message`] = '', 500);
      }, 2500);
    }
  },
